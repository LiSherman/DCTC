...
from dataloaders.dataset import (BaseDataSets, RandomGenerator, WeakStrongAugment, TwoStreamBatchSampler)
...
parser.add_argument("--exp", type=str, default="Fuly", help="method")
...
logdir = f"../logs_acdc/Semi_{args.labeled_num}L_{args.exp}"
print(logdir)
...
labeled_slice = patients_to_slices("ACDC", args.labeled_num)

device = "cuda:0"
db_train = BaseDataSets(base_dir=args.root_path, split="train", transform=transforms.Compose([
        RandomGenerator(image_size,4)
    ]))
...
def create_model(ema=False):
        # model = UNet(in_chns=4, class_num=3)
...
class ACDCTrainer(Trainer):
    ...
    def training_step(self, batch):
        ...
        # Compute losses for labeled data
        loss_dice = self.dice_loss(pred_xstart, L_label)
        pred_xstart = torch.sigmoid(pred_xstart)
        loss_bce = self.bce(pred_xstart, L_label)
        loss_mse = self.mse(pred_xstart, L_label)

        # Process unlabeled data with time-step ensemble
        # Select three time steps based on 1 - dice_loss
        time_steps = get_top_peaks(1-loss_dice.item())
        # Perform sampling at the selected time steps
        ...
        avg_output = (torch.sigmoid(avg_output)>0.5).float()
        step1_output = torch.sigmoid(step1_output)
        step2_output = torch.sigmoid(step2_output)
        step3_output = torch.sigmoid(step3_output)
        consistency_weight = get_current_consistency_weight(self.iter_num//150)
        loss_un = (self.mse(step1_output, avg_output) + self.mse(step2_output, avg_output) + self.mse(step3_output, avg_output))/3*consistency_weight
    ...
    def validation_ds(self, ):
        ...
        for idx, batch in enumerate(val_loader):
            ...
            RV = dice(prediction==1, label ==1)
            Myo = dice(prediction==2, label ==2)
            LV = dice(prediction==3, label ==3)
            print(
